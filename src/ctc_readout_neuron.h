...
#ifndef CTC_READOUT_NEURON_H
#define CTC_READOUT_NEURON_H
...
+++++++++++++++++
 ...
 Readout neuron to be combined with CTC node for training.
 
Description
+++++++++++

 This is a very makeshift implementation. No guarantees, very much hacked style.
 ...
 - CTC -> Readout: for each time step one value per incoming connection, recipient picks
                   its values based on rport.

 This is an implementation of an integrate-and-fire neuron model
...
.. listexamples:: ctc_readout_neuron

EndUserDocs */

using namespace nest;

void register_ctc_readout_neuron( const std::string& name );
 ...
 */
class ctc_readout_neuron : public EpropArchivingNodeReadout< true >
{
...
public:
  ...
  //! Copy constructor.
  ctc_readout_neuron( const ctc_readout_neuron& );
...
private:
  ...
  void ( ctc_readout_neuron::*compute_error_signal )( const long lag );

  //! Map for storing a static set of recordables.
  friend class RecordablesMap< ctc_readout_neuron >;

  //! Logger for universal data supporting the data logging request / reply mechanism. Populated with a recordables map.
  friend class UniversalDataLogger< ctc_readout_neuron >;
  ...
  {
    //! Default constructor.
    Buffers_( ctc_readout_neuron& );

    //! Copy constructor.
    Buffers_( const Buffers_&, ctc_readout_neuron& );
  ...
  //! Get the current value of the symbol probability
  double
  get_p_symbol_() const
  {
    return S_.p_symbol_;
  ...
  static RecordablesMap< ctc_readout_neuron > recordablesMap_;
};

inline long
ctc_readout_neuron::get_shift() const
{
  return offset_gen_ + delay_in_rec_ + delay_rec_out_;
}

inline bool
ctc_readout_neuron::is_eprop_recurrent_node() const
{
  return false;
}

inline size_t
ctc_readout_neuron::handles_test_event( SpikeEvent&, size_t receptor_type )
{
  if ( receptor_type != 0 )
  {
...
inline size_t
ctc_readout_neuron::handles_test_event( CurrentEvent&, size_t receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline size_t
ctc_readout_neuron::handles_test_event( DelayedRateConnectionEvent& e, size_t receptor_type )
{
  ...
  if ( step_rate_model_id == model_id and receptor_type != TARGET_SIG )
  {
    throw IllegalConnection(
      "ctc_readout_neuron neurons expect a connection with a step_rate_generator node through receptor_type "
      "2." );
  }
...
inline size_t
ctc_readout_neuron::handles_test_event( DataLoggingRequest& dlr, size_t receptor_type )
{
  if ( receptor_type != 0 )
  {
    throw UnknownReceptorType( receptor_type, get_name() );
...
inline void
ctc_readout_neuron::get_status( DictionaryDatum& d ) const
{
  P_.get( d );
  S_.get( d, P_ );
...
inline void
ctc_readout_neuron::set_status( const DictionaryDatum& d )
{
